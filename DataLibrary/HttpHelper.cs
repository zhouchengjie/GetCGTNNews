...
namespace GetCGTNNews
{
    public class HttpHelper
    {
        ...
        public static string Response(string url, string postData, HttpCookieCollection cookies, string method = "POST", string charsSet = "utf-8", string contentType = "application/x-www-form-urlencoded", NameValueCollection header = null)
        {
            if (url.Contains("?"))
            {
                url = url + "&date=" + DateTime.Now.ToString("yyyyMMddhhmmss");
            }
        ...
        /// 通过http上传图片及传参数
        /// </summary>
        /// <param name="imgPath">图片地址(D:\demo\img\123.jpg)</param>
        /// <param name="paras">链接参数(para1=1&para2=2&para3=3)</param>
        public static void UploadImage(string imgPath,string paras,string sendUrl)
        {
            var postUrl = string.Format("{0}?{1}", sendUrl, paras);//拼接url
            HttpWebRequest request = WebRequest.Create(postUrl) as HttpWebRequest;
            request.AllowAutoRedirect = true;
            request.Method = "POST";
            request.Timeout = 1000 * 60 * 5; //5分钟
